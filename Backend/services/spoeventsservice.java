...
import com.website.repository.spoeventsrepository;

//defining the business logic
@Service
public class spoeventsservice 
{
@Autowired
spoeventsrepository booksRepository;
//getting all books record by using the method findaAll() of CrudRepository
public List<spoevents> getAllBooks() 
{
List<spoevents> books = new ArrayList<spoevents>();
booksRepository.findAll().forEach(books1 -> books.add(books1));

return books;
}
//getting a specific record by using the method findById() of CrudRepository
public spoevents getBooksById(String id) 
{
return booksRepository.findById(id).get();
}

//saving a specific record by using the method save() of CrudRepository
public void saveOrUpdate(spoevents books) 
{
...
{
	spoevents event = booksRepository.findById(name).get();
	event.name=name;
	event.date=date;
	event.time=time;
