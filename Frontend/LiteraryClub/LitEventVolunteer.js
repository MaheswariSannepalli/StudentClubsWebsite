...
import {useLocation} from 'react-router-dom';
  ...
  const back = {
        ...
        axios.get(`http://localhost:8080/student/${studentid}`)
          .then(res => {
            ...
            if (password === res.data.password) {
              ...
              axios.post(`http://localhost:8080/litvolunteers`, postData)
             .then(res => {
                          console.log(res.data);
                           setStatus(true);
                         })
    ...
    return (
      ...
      {status}
      <Link  to="/Literary" style={back}>GO BACK</Link>
  ...
  export default LiteventVolunteer;