...
import Phobanner from "./Phobanner";
import {
  Link,
  Navigate,
  ...
  function PhotographyPage() {
    ...
    function callApiFunction() {
      ...
      console.log("Function called");
      axios.get(`http://localhost:8080/phoadmin`)
      .then(res => {
          console.log(res.data);
          setBody1(res.data);
    ...
    return (
        <div>
          <Phoheader/>
        <Link to="/Photography/ClubRegistration" style={lin}>Register for Club</Link>
        <Link to="/Photography/Admin">Admin Portal</Link>
  ...
  render() {
      ...
      return (
          <tr class="table_row">
             ...
             <p> <Link to="/Photography/EventRegistration" state={{ eventname: row1.name }} style={divStyle}>Register</Link>
              <Link to="/Photography/EventVolunteer" state={{ eventname: row1.name }}>Volunteer</Link></p>
