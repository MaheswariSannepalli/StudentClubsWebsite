...
import {useLocation} from 'react-router-dom';
  ...
  const back = {
    ...
    function loginStudentApiFunction(e) {
        ...
        axios.get(`http://localhost:8080/spostudent/${studentid}`)
          .then(res => {
            ...
            if (password === res.data.password) {
              ...
              axios.post(`http://localhost:8080/spovolunteers`, postData)
             .then(res => {
                          console.log(res.data);
                           setStatus(true);
                         })
    ...
    return (
      ...
      {status}
      <Link  to="/Sports" style={back}>GO BACK</Link>
  ...
  export default SpoeventVolunteer;