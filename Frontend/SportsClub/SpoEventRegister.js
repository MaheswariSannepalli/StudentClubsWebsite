...
import Footer from '../../footer';
  ...
  const { eventname } = location.state
    ...
    function registerStudentApiFunction(e) {
        ...
        axios.post(`http://localhost:8080/sporegistrations`, postData)
     .then(res => {
    ...
    return (
      ...
      status && <Alert  variant="success">
        ...
        <input type="email" placeholder="Enter email" value={email} onChange={(e)=>setEmail(e.target.value)}/>
                    ...
                    {status && <div className="App" ><Link  to="/Sports">Go Back</Link> </div>}
                </Col>
            </Row></Container>
            <Footer/>
    </div>
  ...
  export default SpoeventRegister;