...
import Robbanner from "./Robbanner";
import {
  Link,
  Navigate,
} from "react-router-dom";
 
  function RoboticPage() {
    ...
    function callApiFunction() {
      ...
      console.log("Function called");
      axios.get(`http://localhost:8080/robadmin`)
      .then(res => {
          console.log(res.data);
          setBody1(res.data);
    ...
    return (
        <div>
          <Robheader/>
        <Link to="/Robotics/ClubRegistration" style={lin}>Register for Club</Link>
        <Link to="/Robotics/Admin">Admin Portal</Link>
  ...
  render() {
      ...
      return (
          <tr class="table_row">
             ...
             <p> <Link to="/Robotics/EventRegistration" state={{ eventname: row1.name }} style={divStyle}>Register</Link>
              <Link to="/Robotics/EventVolunteer" state={{ eventname: row1.name }}>Volunteer</Link></p>
