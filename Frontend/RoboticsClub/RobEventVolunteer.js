...
import {useLocation} from 'react-router-dom';
  ...
  const back = {
    ...
    function loginStudentApiFunction(e) {
        ...
        axios.get(`http://localhost:8080/robstudent/${studentid}`)
          .then(res => {
            ...
            if (password === res.data.password) {
              ...
              axios.post(`http://localhost:8080/robvolunteers`, postData)
             .then(res => {
                          console.log(res.data);
                           setStatus(true);
                         })
    ...
    return (
      ...
      {status}
      <Link  to="/Robotics" style={back}>GO BACK</Link>
  ...
  export default RobeventVolunteer;